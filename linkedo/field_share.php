<?php
	...
	if(isset($_POST['field_id']) && isset($_POST['length'])) {
        ...
        $sql = "select * from lo_share where field_id = ".$field_id." and is_delete = 0 order by date desc limit 0,".$length;
       
        $result = $mysqli->query($sql);
        if($result && $result->num_rows > 0) {
            $arr = $result->fetch_assoc();
            $json_array = array('cid' => '1012','ret' => '1', 'responseData' => $arr);
            echo json_encode($json_array);
            
        } else {
         	$json_array = array('cid' => '1012','ret' => '0');
            echo json_encode($json_array);   
        }
        
    } else {
        $json_array = array('cid' => '1012','ret' => '2');
        echo json_encode($json_array);
    }